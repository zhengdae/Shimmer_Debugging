...
port = '/dev/rfcomm3' # Name of device port
...
def main(args=None):
    ...
    GSR_node.get_logger().info(f'Serial Object Created {ser}')
    ser.flush()
    ...
    while rclpy.ok():
        try:
            ...
            GSR_node.get_logger().error(f'SerialException while streaming GSR data. Error is:\n{e.strerror}')

        except KeyboardInterrupt as key:
            GSR_node.get_logger().info(f"Control C called: {key}")
        ...
        # Publish Messages
        GSR_node.pubImuCalc(imu)
        GSR_node.pubImuRaw(imu_raw)
        # GSR_node.pubMagCalc(mag)
        # GSR_node.pubMagRaw(mag_raw)
        GSR_node.pubGSR(gsr)

        # Log that publishing completed
        GSR_node.get_logger().debug(f'Finished Publishing data to topics')
